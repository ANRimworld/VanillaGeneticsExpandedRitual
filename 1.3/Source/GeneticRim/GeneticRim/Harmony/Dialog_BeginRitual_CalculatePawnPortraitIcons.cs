        ...
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            for (int i = 0; i < codes.Count; i++)
            {
                ...
                if (!found && codes[i].opcode == OpCodes.Call && codes[i].operand == (object)IsAwake)
                {
                    ...
                    yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(GeneticRim_Dialog_BeginRitual_CalculatePawnPortraitIcons_Patch), nameof(IsAwakeNotDead)));
